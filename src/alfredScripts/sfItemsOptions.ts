import { getData, getAlarmWeekDays, getNextTriggerDate, outputAlfredItems } from '../helpers/general';
...
import { cancelOrRestartOrSilenceAlarm, createAlarmOrTimer, deleteAlarm } from '../helpers/helpersAlarm';
...
function getNewAlfredItem(processArgv: string[]): AlfredItem {
  ...
  const alarmWeekDays = getAlarmWeekDays(weekdays);
  const weekDaysFormatted = alarmWeekDays ? `🔃 ${alarmWeekDays.strIsoWeekDays.join(', ')}` : '';
  ...
  const newAlarmItem: NewAlarmOrTimerAttrs = {
    ...
    title: newTitle,
    ...(alarmWeekDays
      ? { type: 'alarmRepeat', isoWeekDays: alarmWeekDays.isoWeekDays }
      : { type: 'alarmOneTime' }),
  };
      ...
      .map((item) => {
        ...
        if (type === 'timer') {
          ...
          const daysFormattedStr =
            type === 'alarmRepeat'
              ? ` 🔃 ${getAlarmWeekDays(item.isoWeekDays.join(''))?.strIsoWeekDays.join(', ')}`
              : '';
